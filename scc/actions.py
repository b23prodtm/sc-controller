...
class Action(object):
	...
	#
	PROFILE_KEY_PRIORITY = 0	# default one. Lower is parsed first
	# PROFILE_KEYS = ("foo", "bar")
	#
	# @staticmethod
	# def decode(jsondata, action, parser, profile_version):
	# 	...
	# 	return action
	...
	def whole_blocked(self, mapper, x, y, what):
		"""
		Special case called when ClickedModifier is used and prevents 'whole'
...
class TiltAction(MultichildAction):
	...
	@staticmethod
	def decode(data, a, parser, version):
...
class XYAction(WholeHapticAction, Action):
	...
	@staticmethod
	def decode(data, action, parser, version):
		""" Called when decoding profile from json """
